...
use ethers::providers::{JsonRpcClient, PendingTransaction, Provider};
use std::sync::Arc;
...
pub struct Submitter<P: JsonRpcClient + 'static> {
...
impl<P: JsonRpcClient + 'static> Submitter<P> {
    pub fn spawn(
    ...
    async fn submit_answer(&mut self, mine_answer: AnswerWithoutProof) -> Result<(), String> {
        ...
        let submission_call = self.mine_contract.submit(answer);
        let pending_transaction: PendingTransaction<'_, P> = submission_call
            .send()
            .await
            .map_err(|e| format!("Fail to send mine answer transaction: {:?}", e))?;

        let _receipt = pending_transaction
