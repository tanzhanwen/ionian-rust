...
class RevertTest(TestFramework):
    ...
    def run_test(self):
        ...
        wait_until(lambda: client.ionian_get_file_info(data_root) is None)
        wait_until(lambda: client.ionian_get_file_info(data_root1) is not None)
        wait_until(lambda: not client.ionian_get_file_info(data_root1)["finalized"])

        segment = submit_data(client, chunk_data)
        self.log.info("segment: %s", segment)
        wait_until(lambda: client.ionian_get_file_info(data_root1)["finalized"])
