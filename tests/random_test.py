...
class RandomTest(TestFramework):
    def setup_params(self):
        ...
        for i in range(self.num_nodes):
            self.ionian_node_configs[i] = {"find_peer_timeout_secs": 1, "confirmation_block_count": 1}
    ...
    def run_test(self):
            ...
            if not no_data:
                ...
                for node_index in range(len(self.nodes)):
                    if node_index != chosen_node:
                        ...
                        wait_until(lambda: self.nodes[node_index].ionian_get_file_info(data_root) is not None)
                        wait_until(lambda: self.nodes[node_index].ionian_get_file_info(data_root)["finalized"], timeout=300)
        ...
        for i in range(tx_count):
            for node in self.nodes:
                status = node.ionian_get_file_info_by_tx_seq(i)
        ...
        for data_root, no_data in file_list:
            if not no_data:
                for node in self.nodes:
                    ...
                    wait_until(lambda: node.ionian_get_file_info(data_root)["finalized"], timeout=300)
